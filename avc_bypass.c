/*
 * avc_bypass — AVC キャッシュ flip (CVE-2023-33107 KGSL UAF) スタンドアロン版 v2
 *
 * 自前で UAF を作成する (cve33107_exploit_sauce の Phase 1-4 を移植):
 *   Phase A1: rbtree setup (UAF gpuobj alloc + CPU unmap, BOGUS, PLACEHOLDER)
 *   Phase A2: import race (KGSL_GPUOBJ_IMPORT vs OVERLAP mmap)
 *   Phase A3: gpuobj_free(UAF) -> dangling GPU PTEs
 *   Phase A4: reclaim (compact_memory + drop_caches)
 * その後 task_struct spray -> SIGKILL -> AVC churn で avc_node を UAF ページに載せ、
 * (tsid==2 && tclass==1) ノードの allowed (avc_node+0xc) に SECURITY__SETENFORCE
 * (bit7=0x80) を立てて setenforce 0 を成功させる (SELinux permissive 化)。
 *
 * 前提: なし (root 不要 — sauce が shell 権限で全 Phase を実行済み)
 *   - /dev/kgsl-3d0 を開けること
 *   - 動作ドメインは shell でも root でも可 (ssid 非依存スキャン)
 *
 * 統合方法:
 *   - 単体実行: adb shell /data/local/tmp/avc_bypass -i
 *   - sauce 連携 (xh 置換): 子の execl 後に自前 UAF を作り直すため exec 制約なし
 *
 * 観測ログ: [UAF] [SPRAY] [TASKPG] [KILL] [FLIP] [SETENFORCE]
 */
